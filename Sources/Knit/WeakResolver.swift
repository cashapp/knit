...
/// A resolver that weakly holds onto the `Swinject.Container`. This allows keeping a reference without the risk of leaking the container
...
public final class WeakResolver {
    ...
    private weak var container: Swinject.Container?
    ...
    public var optionalResolver: Swinject.Resolver? {
        // We are returning `self` rather than the container to maintain weak semantics
        isAvailable ? self : nil
    }
}

// MARK: - Knit.Resolver conformance

extension WeakResolver: Knit.Resolver {

    public var unsafeResolver: any Swinject.Resolver {
        unwrapped
    }

}

// MARK: - Swinject.Resolver conformance

extension WeakResolver: Swinject.Resolver {

    // Force unwraps the weak Container
    // Convenience accessor for private implementation
    private var unwrapped: Swinject.Resolver {
